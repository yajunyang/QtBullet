...
public:
  ...
  class Group
  {
    public:
      ...
      inline Face * pGetFaceHandle(unsigned int face) { return &(faces[face]); }
      inline std::string getName() const { return name; }
  ...
  // ZhangJG
  inline Vec3d* pGetposition(int vertexIndex){return &vertexPositions[vertexIndex];}
  inline Vec3d* pGetPosition(const Vertex & vertex) { return &vertexPositions[vertex.getPositionIndex()]; }
  ...
  inline Group * pGetGroupHandle(unsigned int groupIndex) { return &(groups[groupIndex]); }
